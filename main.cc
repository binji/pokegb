...
uint32_t fb[23040], pal[]={0xffffffff,0xffaaaaaa,0xff555555,0xff000000};
uint64_t bcy, cy=0;
int HA[]={0,0,1,-1};
  ...
  while (1) {
    ...
    if (IME&IF&io[511]) { IF=halt=IME=0;cy+=8;fc=1;U=64;goto CALLU; }
      ...
      case 203: switch((op=r8())) {
        ...
        X7(40) fc=*p&1;FS(0,*p=(int8_t)*p>>1,0,0,fc); break;
      ...
      if (LCDC&128) {
        ...
        if (dot==456) {
          if (LY<144) for (int x=160;--x>=0;) {
            u16 win=LCDC&32&&LY>=io[330]&&x>=io[331]-7, base=(LCDC&(win?64:8)?7:6)<<10;
            u8 mx=win?x-io[331]+7:x+io[323], my=win?LY-io[330]:LY+io[322];
            u16 tile=vram[base+my/8*32+mx/8],p=327;
            mx=(mx^7)&7;
            u8* d=&vram[(LCDC&16?tile:256+(int8_t)tile)*16+(my&7)*2];
